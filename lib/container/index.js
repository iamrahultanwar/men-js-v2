(function (undefined) {
  ...
  var getNested = function getNested(obj, prop) {
    ...
    if (service === undefined && Bottle.config.strict) {
      throw new Error(
        "Bottle was unable to resolve a service.  `" + prop + "` is undefined."
      );
    }
    return service;
  };
    ...
    properties[name] = {
      ...
      get: function getService() {
        ...
        if (provider) {
          ...
          instance = getWithGlobal(decorators, name).reduce(
            reducer,
        ...
        return instance === undefined
          ? instance
          : applyMiddleware(
              getWithGlobal(middlewares, name),
              name,
              instance,
              container
            );
  ...
  var provider = function provider(fullname, Provider) {
    ...
    parts = fullname.split(DELIMITER);
    if (
      this.providerMap[fullname] &&
      parts.length === 1 &&
      !this.container[fullname + PROVIDER_SUFFIX]
    ) {
    ...
    if (parts.length) {
      getNestedBottle
        .call(this, name)
        .provider(parts.join(DELIMITER), Provider);
      return this;
    }
  ...
  var service = function service(name, Service) {
    return createService.apply(
      this,
      [name, Service, true].concat(slice.call(arguments, 2))
    );
  };
  ...
  var serviceFactory = function serviceFactory(name, factoryService) {
    return createService.apply(
      this,
      [name, factoryService, false].concat(slice.call(arguments, 2))
    );
  };
  ...
  var constant = function constant(name, value) {
    ...
    name = parts.pop();
    defineConstant.call(
      parts.reduce(setValueObject, this.container),
      name,
      value
    );
    return this;
  ...
  var register = function register(Obj) {
    ...
    return this[Obj.$type || "service"].apply(
      this,
      [Obj.$name, value].concat(Obj.$inject || [])
    );
  };
  ...
  var resetProviders = function resetProviders(names) {
    ...
    Object.keys(this.originalProviders).forEach(function resetProvider(
